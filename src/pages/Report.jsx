...
export default function Report() {
    ...
    const fetchReports = async () => {
        ...
        try {
            ...
            setError(err.message || 'Failed to load reports list');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            title: "Delete Report?",
            text: "Are you sure you want to delete this report and its files?",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    if (data.success) {
                        ...
                        Swal.fire("Failed!", 'Failed to delete: ' + data.message, "error");
                    }
                } catch (err) {
    ...
    const handleGenerateManual = async (type) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                Swal.fire("Success", `${type} Report successfully generated in the background!`, "success");
                fetchReports();
            } else {
                Swal.fire("Failed!", 'Failed to generate: ' + data.message, "error");
            }
        } catch (err) {
            Swal.fire("Error", 'Error: ' + err.message, "error");
        } finally {
        ...
        <div className="container-fluid py-4">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p className="text-muted mb-0">List of weekly and monthly reports automatically generated by the system.</p>
                </div>
                                ...
                                reports.map((r) => (
                                    <tr key={r.id}>
                                        ...
                                        <td className="py-3 text-slate-600" style={{ fontSize: '0.9rem' }}>
                                            ...
                                            <span className="text-muted" style={{fontSize: '0.8rem'}}>to {formatDate(r.dateEnd)}</span>
                                        </td>
                                        <td className="py-3 text-slate-600" style={{ fontSize: '0.9rem' }}>
